...
#define INVOKE_TARGET_KEY_PUSH "com.example.Sample_messages.invoke.push"

// This needs to match the invoke target specified in bar-descriptor.xml
// The Invoke target key when selecting a notification in the BlackBerry Hub
#define INVOKE_TARGET_KEY_OPEN "com.example.Sample_messages.invoke.open"
...
Q_SIGNALS:
	...
	void createChannelCompleted(const bb::network::PushStatus &status,
			const QString &token);
