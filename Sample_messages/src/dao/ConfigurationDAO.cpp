...
void ConfigurationDAO::save(const Configuration &config)
{
    ...
    settings.setValue(SUCCESS_REGISTER, config.successRegister());
    settings.endGroup();
}
...
Configuration& ConfigurationDAO::configuration()
{
    ...
    m_config.setSuccessRegister(settings.value(SUCCESS_REGISTER, DEFAULT_SUCCESS_REGISTER).toBool());
    settings.endGroup();
...
bool ConfigurationDAO::isSuccessRegister() const {
    bool success;

    QSettings settings;
    settings.beginGroup(QSETTINGS_CONFIG_GROUP);
    success = settings.value(SUCCESS_REGISTER, DEFAULT_SUCCESS_REGISTER).toBool();
    settings.endGroup();
