...
#include "QBContent.hpp"

using namespace bb::cascades;
using namespace bb::system;
using namespace bb::data;

int QBContent::counter = -1;

QBContent::QBContent(bb::cascades::Application *app) :
		QObject(app), m_model(
				new QListDataModel<QObject*>()) {

	qmlRegisterType<QBAuth>("com.example.QBContent.QBAuth", 1, 0, "QBAuth");
...
void QBContent::uploadImage(QString data) {
	qbauth->requestCreateFile(data.mid(data.indexOf("shared"), data.length()));
}

void QBContent::createModel() {
	m_model = new QListDataModel<QObject*>();
}

void QBContent::updateList(QString path) {
	if (path == "") {
		return;
	}
	m_blob_name = "";
...
bb::cascades::DataModel* QBContent::model() const {
	return m_model;
}
 ...
 * Set object
 */
void QBContent::setObjectQBNetwork(QBAuth *qbauth_) {
	qbauth = qbauth_;
}
 ...
 * Get object
 */
QBAuth* QBContent::getObjectQBNetwork() {
	return qbauth;
}
